...
def result_sets_are_same(gen_set, ref_set) -> bool:
    ...
    for item in gen_set:
        if item not in ref_set:
            # logging.info(f"> Missing from reference: {item}")
            return False
    for item in ref_set:
        if item not in gen_set:
            # logging.info(f"> Missing from generated: {item}")
            return False
    return True
...
def answer_no_rag(question: str, model: str):
    ...
    response["messages"] = [{"content": response["content"], "response_metadata": response["response_metadata"]}]
    return response
...
logging.info(f"🧪 Testing {len(example_queries)} queries")
logging.info("## Executing references queries")
...
for i, test_query in enumerate(example_queries):
    ...
    while not res_ref_finally_pass:
        try:
            ...
            logging.info(f"- [x] Reference query {i} '{test_query['question']}' took {time.time() - query_start_time:.2f} seconds")
            ref_results.append(res_from_ref)
            res_ref_finally_pass = True
        except Exception as e:
            logging.info(f"- [ ] Timeout for reference query {i}: {e}, Trying again because we know it should work.")
            res_ref_finally_pass = False
            ...
            for t in range(number_of_tries):
                ...
                try:
                    ...
                    if generated_sparql["query"].strip() == test_query["query"].strip():
                        logging.info(f"✅ {t+1}/{number_of_tries} {test_query['question']}. EXACT MATCH")
                    ...
                    if not result_sets_are_same(res_from_generated, ref_results[query_num]):
                        ...
                        logging.info(f"✅ {t+1}/{number_of_tries} {test_query['question']} = {len(res_from_generated)}")
                        res[approach]["success"] += 1

                except Exception as e:
                    res[approach]["fail"] += 1
                    if approach == "RAG with validation":
                        logging.info(f"❌ {t+1}/{number_of_tries} {test_query['question']}\n{e}")
                        logging.info(f"```sparql\n{generated_sparql['query']}\n```")
                        logging.info("Correct query:")
                        logging.info(f"```sparql\n{test_query['query']}\n````")
...
logging.info(df)
logging.info("\n\n")
logging.info(df.to_csv(os.path.join(bench_folder, f"{file_time_prefix}_tests_results.csv"), index=False))
...
for index, row in df.iterrows():
    ...
    latex_str += row_str + "\n"
with open(os.path.join(bench_folder, f"{file_time_prefix}_tests_results_latex.txt"), "w") as f:
    f.write(latex_str)
