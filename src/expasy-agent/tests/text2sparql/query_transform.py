...
OUTPUT_QUERIES_FILE = os.path.join(
    os.path.abspath(os.path.dirname(__file__)), "queries.csv"
)
...
def transform_text2sparql_queries(input_file: str, endpoint_url: str) -> pd.DataFrame:
    ...
    queries["result"] = queries.apply(
        lambda q: query_sparql(q["query"], q["endpoint"]), axis=1
    )
    queries["result length"] = queries["result"].apply(
        lambda r: len(r["results"]["bindings"]) if "results" in r else 1
    )
    queries["query type"] = queries["query"].apply(
        lambda q: "ASK" if "ASK WHERE" in q.upper() else "SELECT"
    )
...
if __name__ == "__main__":
    ...
    if os.path.exists(OUTPUT_QUERIES_FILE):
        ...
        queries.append(
            transform_text2sparql_queries(
                TEXT2SPARQL_QUERIES_FILE, TEXT2SPARQL_ENDPOINT
            )
        )
        queries.append(
            transform_qald_9_plus_queries(
                QUALD_9_PLUS_QUERIES_FILE, QUALD_9_PLUS_ENDPOINT
            )
        )
        queries.append(
            transform_LC_QuAD_queries(LC_QuAD_QUERIES_FILE, LC_QuAD_ENDPOINT)
        )
