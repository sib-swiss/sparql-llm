...
def plot_queries_by_dataset(queries: pd.DataFrame):
    ...
    ax = sns.countplot(
        ...
        palette=sns.color_palette("tab10")[
            : -(len(queries["query type"].unique()) + 1) : -1
        ],
    )
    ...
    if SAVE_PLOTS:
        plt.savefig(
            os.path.join(bench_folder, f"{file_time_prefix}_queries_by_dataset.png"),
            bbox_inches="tight",
        )
    else:
        plt.show()
...
def plot_result_length(queries: pd.DataFrame):
    queries["result length"] = queries["result length"].apply(
    ...
    if SAVE_PLOTS:
        plt.savefig(
            os.path.join(bench_folder, f"{file_time_prefix}_result_length.png"),
            bbox_inches="tight",
        )
    else:
        plt.show()
...
def plot_triple_patterns(queries: pd.DataFrame):
    ...
    if SAVE_PLOTS:
        plt.savefig(
            os.path.join(bench_folder, f"{file_time_prefix}_triple_patterns.png"),
            bbox_inches="tight",
        )
    else:
        plt.show()
...
def print_error_histogram():
    ...
    csv_file = os.path.join(
        "data", "benchmarks", "20250624_1346_Text2SPARQL_Error_Analysis.csv"
    )
