...
def get_class_info(endpoint_url: str) -> pd.DataFrame:
    ...
    classes = query_sparql(CLASS_QUERY, endpoint_url=endpoint_url)["results"][
        "bindings"
    ]
    classes = (
    ...
    classes["name"] = classes["class"].apply(
    ...
    classes = (
        classes[classes["count"] >= count_threshold]
    ...
    classes["predicates"] = classes["class"].progress_apply(
        lambda c: get_class_predicates(endpoint_url=endpoint_url, class_name=c)
    )
    return classes
...
def get_class_predicates(endpoint_url: str, class_name: str) -> dict[str, str]:
    ...
    predicates = query_sparql(
        PREDICATE_QUERY.format(class_name=class_name, limit=TOP_N_PREDICATES),
        endpoint_url=endpoint_url,
    )["results"]["bindings"]
    predicates = (
        pd.DataFrame(predicates)
        .map(lambda x: x["value"])
        .assign(count=lambda df: df["count"].astype(int))
    )
    ...
    predicates["range"] = predicates["predicate"].apply(
        lambda p: (
            query_sparql(
                DATATYPE_QUERY.format(class_name=class_name, predicate_name=p),
                endpoint_url=endpoint_url,
            )["results"]["bindings"]
            or [{}]
        )[0]
    )
    predicates["range"] = predicates["range"].apply(
        lambda r: r["range"]["value"] if "range" in r.keys() else pd.NA
    )
