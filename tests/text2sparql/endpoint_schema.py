...
class EndpointSchema:
    ...
    def _save_schema_dict(self) -> None:
        ...
        schema = query_sparql(self._CLASS_QUERY.format(graph=self._graph), endpoint_url=self._endpoint_url)["results"][
            "bindings"
        ]
        ...
        schema = (
            schema[schema["count"] >= count_threshold].sort_values(by="count", ascending=False).reset_index(drop=True)
        )
        ...
        schema["predicates"] = Parallel(n_jobs=self._max_workers)(
            delayed(self._retrieve_class_information)(class_name=c)
            for c in tqdm(schema["class"].tolist(), total=len(schema))
        )
    ...
    def _retrieve_class_information(self, class_name: str) -> dict[str, list[str]]:
        ...
        predicates = (
            query_sparql(
                self._PREDICATE_QUERY.format(
                    graph=self._graph, class_name=class_name, limit=self._limit_queries["top_n_predicates"]
                ),
        ...
        predicates = (
            pd.DataFrame(predicates)[["predicate"]].map(lambda p: p["value"] if "value" in p else pd.NA).dropna()
        )

        # Fetch top n ranges for the predicates
        predicates["range"] = predicates["predicate"].apply(
            lambda p: (self._retrieve_predicate_information(class_name, p))
        )
